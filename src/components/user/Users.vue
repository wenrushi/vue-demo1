<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 分页功能 -->
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="queryInfo.pagenum" :page-sizes="[1, 2, 3, 10]" :page-size="queryInfo.pagesize" layout="total, sizes, prev, pager, next, jumper" :total="total"> </el-pagination>
    </el-card>
...
export default {
  data() {
    ...
    var checkEmail = (rule, value, callback) => {
      const resEmail = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/
  ...
  methods: {
    ...
    async removeUserById(id){
      ...
      const confirmRes=await this.$confirm('此操作将永久删除该用户, 是否继续?', '提示', {
        ...
        // 如果用户确认删除，返回的式confirm这个字符串
        // 如果用户取消删除，返回的式cancel这个字符串
